...
<button id="copyCaption" type="button" onclick="(async function(btn){const text=document.getElementById('caption').value||'';if(!text){btn.textContent='❌ 게시문구가 없습니다';setTimeout(()=>btn.textContent='인스타 게시문구 복사',1500);return;}try{await navigator.clipboard.writeText(text);}catch(e){const ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand('copy');ta.remove();}btn.textContent='인스타 게시문구 복사 ✓';setTimeout(()=>btn.textContent='인스타 게시문구 복사',1500);})(this)">인스타 게시문구 복사</button>
...
app.get("/facebook-personal-share", (req, res) => {
  // Prefer the Reel ID returned by the Facebook publish job for this exact
  // browser session. This prevents the button from losing the newly-created
  // Reel when the server restarts or the page is refreshed.
  const requestedVideoId = String(req.query.video_id || "").trim();

  if (requestedVideoId) {
    return res.redirect(
      "https://www.facebook.com/reel/" +
      encodeURIComponent(requestedVideoId) +
      "/"
    );
  }

  // Fallback to the current in-memory Reel from this running instance.
