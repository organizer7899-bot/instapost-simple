...
<button id="post" type="button" onclick="(async function(btn){const file=document.getElementById('video').files[0],result=document.getElementById('result'),info=document.getElementById('fileInfo'),caption=document.getElementById('caption').value||'';if(!file){result.textContent='❌ 영상을 먼저 선택하세요.';return;}btn.disabled=true;btn.textContent='⏳ 게시 처리 중...';result.textContent='① 게시 버튼 작동 — 영상 업로드 중...';info.textContent='📤 '+file.name+' — 서버로 전송 중...';try{const fd=new FormData();fd.append('video',file);fd.append('caption',caption);const response=await fetch('/publish',{method:'POST',body:fd});const data=await response.json();if(!response.ok)throw new Error(data.error||'게시 실패');result.textContent='② Instagram 게시 완료 — Facebook 릴 게시 중...';info.textContent='✅ Instagram 게시 완료 · Facebook 릴 게시 중...';if(data.facebook&&data.facebook.pending&&data.facebook.jobId){for(let i=0;i<90;i++){await new Promise(r=>setTimeout(r,2000));const jr=await fetch('/facebook-job/'+encodeURIComponent(data.facebook.jobId)+'?t='+Date.now(),{cache:'no-store'});const jd=await jr.json();if(jd.status==='done'){info.textContent='✅ Instagram + Facebook 게시 완료';result.textContent='③ Instagram + Facebook 동시 게시 완료';break;}if(jd.status==='error'){info.textContent='⚠️ Instagram 게시 완료 · Facebook 실패';result.textContent='⚠️ Facebook: '+(jd.error||'게시 실패');break;}result.textContent='② Instagram 게시 완료 — Facebook 릴 게시 중... ('+(i+1)+'/90)';}}else{info.textContent='⚠️ Instagram 게시 완료';result.textContent='③ Instagram 게시 완료';}}catch(e){info.textContent='❌ 게시 실패';result.textContent='❌ '+e.message;}finally{btn.disabled=false;btn.textContent='Instagram + Facebook에 게시';}})(this)">Instagram + Facebook에 게시</button>
