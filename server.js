...
const flows=new Map(); let client=null, accessToken="", connectedUser="";
const EXPECTED_USER="thkimtw";
const BASE=(process.env.BASE_URL||"").replace(/\/$/,"");
app.get("/",(_,res)=>res.send(`<!doctype html><meta name="viewport" content="width=device-width"><title>X Publisher</title><style>body{font-family:sans-serif;max-width:650px;margin:40px auto;padding:20px}button,input,textarea{font-size:18px;margin:8px 0;padding:12px;width:100%;box-sizing:border-box}button{background:#111;color:#fff;border:0;border-radius:10px}</style><h1>X Publisher</h1><p>게시 대상: @THKimtw</p><p>연결 상태: ${connectedUser ? "@"+connectedUser : "연결 필요"}</p><p>동영상 업로드 → X 게시</p><a href="/login"><button>X 계정 연결</button></a><form action="/post" method="post" enctype="multipart/form-data"><input type="file" name="video" accept="video/*" required><textarea name="text" rows="5" maxlength="280" placeholder="게시 문구"></textarea><button>동영상 X에 게시</button></form>`));
app.get("/login",(_,res)=>{try{const c=new TwitterApi({clientId:process.env.X_CLIENT_ID,clientSecret:process.env.X_CLIENT_SECRET});const state=crypto.randomBytes(24).toString("hex");const x=c.generateOAuth2AuthLink(BASE+"/callback",{scope:["tweet.read","tweet.write","users.read","media.write","offline.access"],state});flows.set(state,{verifier:x.codeVerifier,at:Date.now()});res.redirect(x.url)}catch(e){res.status(500).send("X 로그인 시작 실패: "+e.message)}});
app.get("/callback",async(req,res)=>{try{if(req.query.error)return res.status(400).send("X 승인 실패: "+req.query.error);const flow=flows.get(req.query.state);if(!flow)return res.status(400).send("로그인 세션이 만료되었습니다. X 계정 연결을 다시 눌러주세요.");flows.delete(req.query.state);const c=new TwitterApi({clientId:process.env.X_CLIENT_ID,clientSecret:process.env.X_CLIENT_SECRET});const x=await c.loginWithOAuth2({code:String(req.query.code||""),codeVerifier:flow.verifier,redirectUri:BASE+"/callback"});const me=await x.client.v2.me();const username=me.data?.username||"";if(username.toLowerCase()!==EXPECTED_USER)return res.status(403).send("연결된 X 계정 @"+username+"은 게시 대상 @THKimtw가 아닙니다. X에서 @THKimtw로 로그인한 뒤 다시 연결하세요.");client=x.client;accessToken=x.accessToken;connectedUser=username;res.redirect("/?connected=1")}catch(e){console.error("oauth callback",e?.data||e);res.status(500).send("X 연결 실패: "+(e?.data?.detail||e?.data?.title||e.message))}});
...
app.post("/post",upload.single("video"),async(req,res)=>{let path=req.file?.path;try{if(!client||connectedUser.toLowerCase()!==EXPECTED_USER)return res.status(401).send("@THKimtw X 계정을 먼저 연결하세요.");if(!req.file)return res.status(400).send("동영상을 선택하세요.");
 ...
 if(pi && pi.state!=="succeeded")throw new Error("X 동영상 처리 시간이 초과되었습니다. 잠시 후 다시 시도하세요.");
...
}catch(e){console.error("post failed",e?.data||e);res.status(500).send("게시 실패: "+(e?.data?.detail||e.message)+(e.message?.includes("X API 403")?" X 계정을 다시 연결해 media.write 권한을 승인하고 X 개발자 앱의 미디어 업로드 접근 권한을 확인하세요.":""))}finally{if(path)fs.unlink(path,()=>{})}});
